...
public class GalacticSearchA extends CommandBase {
  ...
  public GalacticSearchA(DriveTrain driveTrain, Intake intake, Spindexer spindexer, Ejector ejector) {
    // Initialize the Auto Intake Command
    autoIntake = new AutoIntake(intake, spindexer, ejector);

    // Initialize the Path Commands
    redPath = new AutoFollowPath(driveTrain, new GalacticSearchARed(driveTrain).generateSwerveTrajectory());
    bluePath = new AutoFollowPath(driveTrain, new GalacticSearchABlue(driveTrain).generateSwerveTrajectory());
